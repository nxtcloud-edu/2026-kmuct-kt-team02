...
| `turn` | 필수 | 아래 세 형태 중 하나 |

`turn`은 `type`으로 구분한다.

| `type` | 추가 필드 | 의미 |
| --- | --- | --- |
| `message` | `message` (비어 있지 않은 문자열) | 일반 사용자 발화 |
| `followup_answer` | `field`, `value` | 후속 질문 선택값 |
| `followup_skip` | `field` | 후속 질문 건너뛰기 |
...
- **프로필 항목이 아닌 질문의 답은 프로필에 넣지 않는다.** 기준 시점을 묻는 확인 질문의 `field` 는 `planned_basis` 이고 값은 `planned` 또는 `current`다. 이 답은 세션이 평가용 프로필에만 적용하며 실제 현재 프로필에는 넣지 않는다. `followup_skip`은 `current` 기준을 유지하고 `planned_basis`를 다시 묻지 않는다.

**돌려주는 것**: 스트리밍 이벤트. 5장.

**검증**: 공통 필드·`turn` 모양·허용 값이 틀리면 입력 오류다. 세션 없음·만료는 `session_expired` 404, 같은 `client_message_id` 재전송은 `duplicate_request` 409, 세션당 1분 10회 초과는 `rate_limited` 429다. 이 오류들은 스트림을 열기 전에 JSON envelope로 돌려준다.
...
**조건 요약 키는 `name`으로 확정한다.** `summary`는 공개 API·SSE·AI B 출력에서 허용하지 않는다. 모든 조건 결과는 `name`, `met`·`unmet`·`unknown`, `rule`·`ai`의 영문 고정 값을 쓴다.
...
스트림을 연 뒤 timeout·검증 실패·예상치 못한 예외가 나면 안전한 `{ code, message }` `error` 프레임을 한 번 보내고 반드시 `done`으로 닫는다. 전체 요청 상한은 20초다.
...
| 전체 시간 초과 | `timeout` | 응답이 오래 걸려 여기까지의 결과를 보여드려요 |
| 중복 요청 | `duplicate_request` | 같은 요청을 이미 처리하고 있어요 |
| 요청 제한 | `rate_limited` | 요청이 많아요. 잠시 후 다시 시도해 주세요 |
...
- 보관 내용
  ...
  - 보류된 예정 변경과 선택한 기준(`planned`/`current`). `planned`일 때만 평가용 프로필에 overlay하며 현재 프로필은 유지한다
  - 처리한 `client_message_id` 최근 32개 — 중복 요청을 막는다
- 30분 동안 요청이 없으면 폐기
...
로그에는 메시지 원문, 마스킹 전후 값, 프로필 전체를 남기지 않는다.
