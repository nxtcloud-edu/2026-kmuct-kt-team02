"""예외 조건 판정 채점 (`ai/judgment/README.md` 10장, 12장).
...
`unmet` 오판은 기대가 `unmet` 이 아닌데 `unmet` 으로 판정한 경우다.
받을 수 있는 제도가 "어려울 수 있어요"로 접힌 영역에 들어가고 사용자는 그 제도를
포기한다. 되돌릴 수 없으므로 따로 센다.

반대 방향(`unmet` 인데 `unknown` 으로 본 것)은 질문이 하나 늘거나 조건부 문장이
하나 붙는 정도다. 안전한 쪽 오차로 구분해 기록한다.

채점 대상
---------
판정기 출력을 그대로 채점하지 않는다. **인용 검증을 통과한 뒤의 결과**를 채점한다.
판정이 맞아도 발췌가 원문에 없으면 실제로는 `unknown` 으로 강등되므로,
검증 전 출력으로 채점하면 본선 동작과 어긋난 수치가 나온다.
...
돌려주는 각 항목은 `docs/03-api-contract.md` 4-1 형식이다.
``{"name": str, "result": str, "judged_by": "ai", "excerpt": str, "needed_field": str | None}``
"""
...
# 판정 상태 계산(`docs/01-glossary-profile.md` 4장)에서 unmet 이 우선하는 것과 같은 방향이다.
...
class Report:
    ...
    def removed_excerpt_count(self) -> int:
        """인용 검증에서 제거된 발췌 수. 지표 항목 (README 12장)."""
    ...
    def summary(self) -> str:
        """지표 슬라이드에 그대로 옮길 수 있는 형태 (README 12장)."""
        lines = [
            f"예외 조건 판정 평가: {self.passed}/{self.total} 통과",
            f"unmet 오판: {self.false_unmet}건 (완료 기준 0건)",
            f"안전한 쪽 오차(unmet→unknown): {self.safe_misses}건",
        ...
        if self.placeholder_cases:
            lines.append(
                ...
                "(source_url 미기입). 이 상태의 수치는 발표에 쓰지 않는다"
            )
    ...
    def detail(self) -> str:
        ...
        for o in self.outcomes:
            ...
            row = (
                ...
                f"       기대 {o.case.expected_result} / 실제 {o.actual_result}"
            )
            ...
            if o.is_false_unmet:
                row += "\n       >> unmet 오판. 받을 수 있는 제도를 접어 버리는 경우"
...
def _representative(conditions: Sequence[Dict[str, object]]) -> Dict[str, object]:
    ...
    가장 심각한 결과를 대표로 삼는다. 판정 상태 계산에서 `unmet` 이 우선하는 것과
...
def evaluate_case(judge: Judge, case: ExceptionCase) -> CaseOutcome:
    ...
    except Exception as exc:  # 판정기 실패도 평가 대상이다 (README 9장)
        return CaseOutcome(
            case=case,
            actual_result=UNKNOWN,
            actual_needed_field=ASK_NOTICE,
    ...
    if case.expected_needed_field is None:
        # `unknown` 이 아니면 필요한 추가 항목은 의미가 없다.
