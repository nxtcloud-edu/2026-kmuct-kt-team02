...
| `CLAUDE_MODEL` | `DEFAULT_MODEL` | 모델 이름. **설정에서 바꾼다** (아래 주석 참고) |
...
# 모델 이름은 **설정에서 바꾼다.** 이 값은 환경 변수가 없을 때만 쓰는 자리표시다.
# 본선 당일 10:30 에 캠프에서 받는 모델 이름을 `CLAUDE_MODEL` 로 넣는다.
# 코드를 고치지 말고 환경 변수를 고친다. 모델 이름을 다른 파일에 흩지 않는다.
...
# 표 그대로. 여기 없는 종류는 재시도하지 않는다.
RETRYABLE_KINDS = frozenset({KIND_SERVER, KIND_SCHEMA})
...
def load_config(env: Optional[Mapping[str, str]] = None) -> ClaudeConfig:
    ...
    model = _read(source, ENV_MODEL) or DEFAULT_MODEL
...
class ClaudeClient:
    ...
    def _create(self, sdk: Any, *, system: str, user: str, timeout: float) -> Any:
        ...
        try:
            ...
            # timeout 인자를 받지 않는 SDK·가짜 객체. 네트워크는 아직 타지 않았으므로
            # 이 재호출은 API 재시도가 아니다.
