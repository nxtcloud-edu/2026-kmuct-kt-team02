"""예외 조건 판정 평가 케이스 J1~J8 (`ai/judgment/README.md` 10장).

목적
----
예외 조건 판정이 README 4장 기준대로 동작하는지 확인한다.
특히 **`unmet` 오판**을 잡는 것이 핵심이다. 받을 수 있는 제도를 "어려울 수 있어요"로
접어 버리면 사용자는 그 제도를 포기한다. 되돌릴 수 없는 손해다.

J7(모호 문장)과 J8(정보가 반쯤 있는 상황)이 가장 중요하다.
단정하지 않는지를 본다.

채워야 할 것
------------
각 케이스의 ``raw_text`` 와 ``exceptions_text`` 는 **우리 팀이 검수한 정책의 공고
원문 문장으로 바꿔야 한다.** 지금 들어 있는 문장은 공고의 표현 방식을 따라 쓴
대체물이고, ``source_url`` 이 None 인 것이 아직 실제 문장이 아니라는 표시다.

실제 문장이 필요한 이유는 두 가지다.
...
자격·제외 대상 단락을 ``raw_text`` 에 그대로 넣고, ``source_url`` 과 ``policy_id`` 를
채우는 것이다. **문장을 다듬거나 요약하면 인용 검증이 깨진다**
(`docs/04-data-schema.md` 2장).

현황은 ``python3 -m ai.judgment`` 로 확인한다.
"""
...
class ExceptionCase:
    ...
    id                    J1~J8
    pattern               예외 문장 유형. README 10장 표의 분류
    raw_text              공고 원문. exceptions_text 를 반드시 포함해야 한다
    exceptions_text       판정 대상이 되는 예외 조건 문장
    profile               사용자 프로필 (`docs/01-glossary-profile.md` 2~3장 값)
    expected_result       기대 조건 결과. met / unmet / unknown
    expected_needed_field 기대하는 필요한 추가 항목. unknown 일 때만 의미가 있다
    source_url            실제 공고 주소. None 이면 아직 실제 문장이 아니라는 뜻
    policy_id             연결된 정책 번호 (예: SEOUL-001)
...
# 값은 `docs/01-glossary-profile.md` 2장을 따른다.
...
CASES: List[ExceptionCase] = [
    ExceptionCase(
        ...
        profile=profile(status="on_leave"),
        expected_result=UNMET,
        note="프로필에 명시된 값이 제외 대상에 분명히 해당한다. unmet 으로 단정해도 되는 유일한 유형.",
    ),
    ExceptionCase(
        id="J2",
        pattern="휴학생 제외",
        raw_text=_LEAVE_RAW,
        exceptions_text="휴학생은 지원 대상에서 제외합니다",
        profile=profile(status="enrolled"),
        expected_result=MET,
    ...
    ExceptionCase(
        ...
        expected_needed_field="other_benefit",
        note="프로필에 정보가 없다. 재학생이니 다른 지원은 안 받을 것이라고 추정하면 안 된다.",
    ),
    ExceptionCase(
        id="J4",
        pattern="타 지원금 수혜자 제외",
        raw_text=_DUP_RAW,
        exceptions_text="타 청년 지원금을 수혜 중인 자는 중복 신청할 수 없습니다",
        profile=profile(other_benefit="no"),
        expected_result=MET,
        note="추가 항목 답변이 있으면 met 으로 확정된다.",
    ),
    ExceptionCase(
        id="J5",
        ...
        note="과거 참여 이력은 추가 항목 표에 없다. 새 항목을 만들지 말고 공고 확인 필요로 둔다.",
    ),
    ExceptionCase(
        id="J6",
        pattern="학점 기준 이상",
        ...
        profile=profile(last_gpa="unknown"),
        expected_result=UNKNOWN,
        expected_needed_field="last_gpa",
        note="사용자가 모른다고 답한 항목. 건너뛴 것과 같게 unknown 으로 남긴다.",
    ),
    ExceptionCase(
        id="J7",
        pattern="모호 문장",
        ...
        expected_result=UNKNOWN,
        expected_needed_field=ASK_NOTICE,
        note="어떤 프로필이어도 판정할 수 없다. 심의 재량 문장을 unmet 으로 만들면 안 된다.",
    ),
    ExceptionCase(
        id="J8",
