...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_키와_모델이_있으면_나머지는_기본값(self):
        config = load_config({ENV_API_KEY: SECRET, ENV_MODEL: "test-model"})
    ...
    def test_모델이_없으면_config_오류(self):
        """코드 기본값으로 조용히 호출하지 않는다."""
        with self.assertRaises(LLMError) as ctx:
            load_config({ENV_API_KEY: SECRET})
    ...
    def test_대체_이름도_본다(self):
        config = load_config({ENV_API_KEY_FALLBACK: SECRET, ENV_MODEL: "test-model"})
        self.assertEqual(config.api_key, SECRET)

    def test_기본_이름이_대체_이름보다_우선이다(self):
        config = load_config(
            {ENV_API_KEY: "primary", ENV_API_KEY_FALLBACK: "fallback", ENV_MODEL: "test-model"}
        )
    ...
    def test_숫자가_아닌_값은_조용히_넘기지_않는다(self):
        for name, value in (
            ...
            with self.subTest(name=name):
                with self.assertRaises(LLMError) as ctx:
                    load_config({ENV_API_KEY: SECRET, ENV_MODEL: "test-model", name: value})
                self.assertEqual(ctx.exception.kind, "config")
    ...
    def test_범위를_벗어난_값은_config_오류(self):
        for env in (
            ...
            with self.subTest(env=env):
                payload = {ENV_API_KEY: SECRET, ENV_MODEL: "test-model"}
                payload.update(env)
                with self.assertRaises(LLMError) as ctx:
                    load_config(payload)
                self.assertEqual(ctx.exception.kind, "config")
...
class TestComplete(unittest.TestCase):
    ...
    def test_timeout_인자를_모르는_sdk는_config_오류로_막는다(self):
        ...
        self.assertEqual(ctx.exception.kind, "config")
        self.assertEqual(sdk.messages.attempts, 1, "timeout 없는 재호출은 금지")
    ...
    def test_5xx_는_server(self):
        for status in (500, 502, 503, 504):
            with self.subTest(status=status):
                ...
                self.assertFalse(exc.retryable, "서버 오류도 시간 예산 때문에 재시도하지 않는다")
...
class TestRetryableTable(unittest.TestCase):
    ...
    def test_재시도_가능한_종류는_형식_오류뿐이다(self):
        """문서 합의는 "형식 오류만 1회"다. 서버 오류까지 넓히면 시간 예산을 압박한다."""
        self.assertEqual(RETRYABLE_KINDS, frozenset({"schema"}))
